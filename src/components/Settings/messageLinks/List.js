...
import CreateImport from '../trainbot/CreateImport';
  ...
  const inputElement = useRef([]);
   ...
   const deleteRow = (id) => {
      CrudService.deleteRow(id, 'helpcustomfield', true).then(
         (response) => {
            toast('Custom Field has been deleted!', { type: toast.TYPE.SUCCESS });
...
const selCat = (e) => {
 ...
 const importData = () => {
   ...
   loadModal('Create Link',<LinkModal closeModal={closeModal} title="Create Link" retrieveList={retrieveList} loadList={loadList} />)
}
                     ...
                     <table className="table table-hover" id="settingTbl">
                        ...
                        <div className='qr-link-container column text-center mt-5 pt-5'>
                           ...
                           <div className='qr-link-item'>
                              ...
                              <button class="primary" onClick={() => importData()}>
                                 Create message link
                              </button>
